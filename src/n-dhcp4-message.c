...
int n_dhcp4_outgoing_new(NDhcp4Outgoing **outgoingp, size_t max_size, uint8_t overload) {
        ...
        static_assert(N_DHCP4_MESSAGE_MIN_SIZE >= sizeof(NDhcp4Message) + 1, "Invalid min message size");

        outgoing = calloc(1, sizeof(*outgoing));
        if (!outgoing)
                return -ENOMEM;

        outgoing->n_message = N_DHCP4_MESSAGE_MIN_SIZE;
        ...
        outgoing->message->magic = htobe32(N_DHCP4_MESSAGE_MAGIC);

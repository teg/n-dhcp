...
_public_ NDhcp4Client *n_dhcp4_client_free(NDhcp4Client *client) {
        ...
        if (client->rfd >= 0) {
                epoll_ctl(client->efd, EPOLL_CTL_DEL, client->rfd, NULL);
                close(client->rfd);
        }
        if (client->tfd >= 0) {
...
static int n_dhcp4_client_dispatch_rfd(NDhcp4Client *client, unsigned int events) {
        if (events & EPOLLIN) {
                /* XXX */
...
_public_ int n_dhcp4_client_dispatch(NDhcp4Client *client) {
        ...
        if (r > 0) {
                ...
                else if (ev.data.fd == client->rfd)
                        r = n_dhcp4_client_dispatch_rfd(client, ev.events);
